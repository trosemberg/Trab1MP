...
int soma_string(char * string_entrada ){
	int tamstr = 0,soma = 0,ndigitos = 0,nnum = 0,sep=1;
	...
	/*a varieavel sep, server pra contar o separador,se encontrar um separador ela é setada para 1, se encontrar um numero ela é setada para 0, seta forma é possivel 
	...
	for (int i = 0; i < tamstr; i++){
		if((string_entrada[i]>='0') && (string_entrada [i]<='9')){
			...
			return -1;
				...
				return -1;
		}else if(string_entrada[i]==',' && sep == 1){
			return -1;
		}else{
			if(ndigitos<4){
